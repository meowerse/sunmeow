...
namespace video {
  ...
  void encode_run(
    ...
    meow::viewport::encode_hook_t<platf::img_t> meow_viewport;  // MEOW-TOUCH(viewport): echo from the encode path, re-convert when idle
    ...
    while (true) {
      ...
      if (!requested_idr_frame || images->peek()) {
        if (auto img = images->pop(max_frametime)) {
          ...
          meow_viewport.remember(img);  // MEOW-TOUCH(viewport)
      ...
      if (meow_viewport.refresh(*session)) {  // MEOW-TOUCH(viewport): a pan on an idle desktop
        BOOST_LOG(error) << "Could not convert image"sv;
        return;
      }
      ...
      meow_viewport.encoded(frame_nr - 1);  // MEOW-TOUCH(viewport): echo names this frame
