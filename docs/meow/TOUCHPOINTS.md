...
| `src/nvhttp.cpp` | `MEOW-TOUCH(uniqueid)` | **`20` added, `1` deleted (the replaced assignment)** — inside `load_state()`, the single `http::unique_id = std::move(*unique_id_p);` line becomes an `if (unique_id_p->empty())` branch that regenerates the id and then **keeps loading**. Layers 1–3 fail: `load_state()` is a free function whose parsed `ptree` never leaves it, and the assignment sits between the absence check and the `named_devices` loop, so the decision has to be made in that exact place. **Why it exists:** `get_optional` returns an engaged optional for `"uniqueid": ""`, so upstream's absence-only check kept the id empty forever and `save_state()` re-serialised it empty; `/serverinfo` then served `<uniqueid/>`, which Moonlight rejects as a missing mandatory field — the host could never be added or paired. The early `return` stays reserved for a *missing* key: taking it for an empty one would drop every paired client, which `save_state()` would then persist. Pinned by `tests/unit/test_nvhttp_unique_id.cpp`, including `EmptyUniqueIdRecoveryPreservesPairedClients`. | 2026-09-24 |
| `tests/tests_main.cpp` | `MEOW-TOUCH(test-isolation)` | **`74` added, `1` deleted (the replaced `return RUN_ALL_TESTS();`)** — a `constructor(101)` function that clears `CONFIGURATION_DIRECTORY` and `SUNSHINE_MIGRATE_CONFIG` and points `XDG_CONFIG_HOME` at a per-pid sandbox under `$TMPDIR`, plus its removal after `RUN_ALL_TESTS()`. Layers 1–3 fail on *ordering*, not structure: `src/config.cpp` initialises `APPS_JSON_PATH` at namespace scope through `platf::appdata()`, which memoizes with `std::call_once`, so the redirection must run before the unprioritised dynamic initialisers — a gtest `Environment`, a new TU's static object or anything in `main()` all run too late (the first attempt did, and the guard test caught it). **Why it exists:** suite runs twice destroyed a real installation's pairings by writing the live `~/.config/sunmeow/sunmeow_state.json`. `tests/unit/test_config_dir_isolation.cpp` fails if `appdata()` ever resolves inside the user's real config tree again. | 2026-09-24 |
