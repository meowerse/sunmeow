...
now returns **12** marker families (`adaptive-bitrate`, `ccache-scope`, `cmake-deps`,
`green-brand`, `rebrand`, `test-sandbox`, `unified-desktop-capture`, `uniqueid`, `viewport`,
`viewport-cuda`, `web-deps`, `web-identity`) across **66** distinct (file, marker) pairs in
**60** files, as of 2026-09-24 on the `web-identity` branch after merging #18. (An earlier
revision listed a `clipboard` family; no such marker exists — it is only the example in
`CLAUDE.md` §3.)
