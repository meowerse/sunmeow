...
now returns **8** marker families (`adaptive-bitrate`, `ccache-scope`, `green-brand`,
`rebrand`, `unified-desktop-capture`, `viewport`, `viewport-cuda`, `web-deps`) across **53**
distinct (file, marker) pairs in **47** files, as of 2026-09-24 (after the sync and
`green-brand`). `cmake-deps` was retired by that sync. The previous revision also listed a
`clipboard` family; no such marker exists in the tree — `clipboard` is only the example in
`CLAUDE.md` §3, which the regeneration command below excludes.
...
justified above: `vite.config.js` (`10`, the codecov removal), `package.json` (`6`),
