...
Verified 2026-09-26 against `origin-upstream/master` (Sunshine `e1e6700b`), immediately after
the sync that merged 9 upstream commits: PipeWire pts passthrough for whitelisted compositors
with a zero/stale-pts guard (`#5791`), and portal fallback capture with KWin probed ahead of
portal and the legacy `CAP_SYS_ADMIN` workarounds removed (`#5774`). **Only
`pipewire.cpp` and `kwingrab.cpp` conflicted, and both hunks were adjacency, not overlap** —
each side's lines were kept verbatim, so every `--numstat` count against upstream in this file
is unchanged from 2026-09-24 (`kwingrab.cpp` 287/8, `pipewire.cpp` 41/1, `misc.cpp` 24/4).
What the sync changed in the rows below: the `cursor` hook in `pipewire.cpp`'s `fill_img()`
now passes upstream's new `prefer_pipewire_pts` into `fill_memory_img()`, which applies
upstream's timestamp rule to the metadata-mode path (see that row); `kwingrab.cpp`'s third
xdg-output roundtrip now runs before upstream's new `is_kwin_screencasting_available()`
check in `screencast_t::init()`; and upstream's removal of the `CAP_SYS_ADMIN` dummy-display
branch in `kwin_display_names()` was checked against whole-desktop capture, which never read
it (the dummy was a single `""` name that the `all` pseudo-display does not depend on). One behaviour change to know about: a *privileged* run (`setcap`, i.e. a KMS setup) now enumerates the real KWin outputs during encoder probing - including `all` - where it used to get the dummy; the owner's unprivileged `capture = kwin` host never took that branch.

Previously verified 2026-09-24 against `origin-upstream/master` (Sunshine `c48e50e4`), immediately
...
- **`src/platform/linux/kwingrab.cpp` does not have a `0` deletion count** — it is `282` added (`287` as of 2026-09-26),
...
Ten upstream **C++ sources** are now modified: two by `unified-desktop-capture`
(`kwingrab.cpp`, `misc.cpp`), four — `config.h`, `config.cpp`, `stream.cpp`, `video.cpp` —
by `adaptive-bitrate`, `viewport` and (since 2026-09-24) `cursor`/`meow-control`, three by
`viewport-cuda` (`cuda.h`, `cuda.cu`, `cuda.cpp`), and `pipewire.cpp` by `cursor`
(2026-09-24; `kwingrab.cpp` carries `cursor` markers too). Derive the current list with
`git grep -l 'MEOW-TOUCH' -- src/` rather than trusting this sentence.
...
| --- | --- | --- | --- |
| `packaging/linux/meow.alxnko.sunmeow.desktop` | `MEOW-TOUCH(kwin-launcher-permission)` | **Found on hardware 2026-09-26, after the root install was removed.** KWin 6 identifies a Wayland client started by the user unit `app-meow.alxnko.sunmeow.service` through the unit's app id and checks `X-KDE-Wayland-Interfaces` on *that* desktop file — the launcher — not on `meow.alxnko.sunmeow.kwin.desktop`, which only matches by `Exec` for processes started outside the unit (a shell, the isolated test host). Symptom: the service logs `Unable to initialize capture method` / `Unable to find display or encoder` within milliseconds of start while the very same binary captures fine from a shell. Upstream's `dev.lizardbyte.app.Sunshine.desktop` has the same gap (its unit is `app-dev.lizardbyte.app.Sunshine.service`). The key is a data file edit with no layer-1–3 alternative; it grants the same single interface the `.kwin` entry already grants the same binary. | 2026-09-26 |
...
| `src/platform/linux/kwingrab.cpp` | `MEOW-TOUCH(cursor)` | **2026-09-24.** A `pointer_mode` member on `screencast_t` (default `POINTER_EMBEDDED`, upstream's value) passed to the two `stream_output`/`stream_region` calls in place of the literal, and four lines in `kwin_t::configure_stream()` that decide metadata mode for the memory path `pipewire_display_t` detected, choose `POINTER_METADATA` accordingly, and log which mode is used. The two call lines were already among this file's 8 in-place edits, so the deletion count is unchanged. Layers 1-3 fail for the same reason as the row above: `screencast_t` is private to this file. The decision is `meow::cursor::metadata_mode_wanted()`, tested in `MeowCursorRuntime.*`. | 2026-09-24 |
| `src/platform/linux/pipewire.cpp` | `MEOW-TOUCH(cursor)` | **2026-09-24, 41 added, 1 deleted** (an earlier revision printed 35; `--numstat` said 41 before and after the 2026-09-26 sync) (the `params` array of the buffer negotiation grew from 3 to 4 to hold the `SPA_META_Cursor` request). An include; a `meow::cursor::pipewire::stream_t meow_cursor` member on `stream_data_t`; a setter on `pipewire_t`; a `meow_cursor_memory_path` flag computed in `pipewire_display_t::init()` before `configure_stream()` and a `meow_cursor_metadata` flag that **only `kwin_t::configure_stream()` sets** (portal capture never asks for cursor metadata, so it never enters this path); and three one-branch hooks - `on_process()` hands memory buffers to `meow::cursor::pipewire::process_memory()`, `fill_img()` hands out the front buffer with the cursor drawn in via `fill_memory_img()`, and `on_param_changed()` requests the cursor metadata or, for a format the blend cannot write (or DMA-BUF), gives up on metadata mode and restarts the capture with the cursor embedded. **Every hook is inert unless metadata mode was chosen**, so portal capture and every DMA-BUF path run upstream's code unchanged. Layers 1-3 fail because `stream_data_t`, `pipewire_t` and the callbacks are private to this file and are not virtual. Why the metadata-mode memory path is ours rather than upstream's: in that mode KWin sends a `SPA_CHUNK_FLAG_CORRUPTED` buffer on every pointer motion, which upstream's path would copy into the staging buffer, and upstream's `fill_img()` reads chunk metadata from a buffer it already returned to PipeWire, which KWin reuses for exactly those updates - see `src/meow/cursor_pipewire.h`. Parsing, the blend and the restore are pure and tested pixel-exact in `tests/unit/meow/test_cursor.cpp`. **2026-09-26 sync:** upstream `#5791` made `fill_img_metadata()` stamp frames with the compositor's PipeWire pts on whitelisted compositors (`pipewire_t::prefer_pipewire_pts`) and treat a pts of `0` as absent. Our memory path replaces that function for metadata-mode streams, so the `fill_img()` hook now passes `prefer_pipewire_pts` to `fill_memory_img()`, which picks the timestamp with the same rule (`meow::cursor::pipewire::frame_timestamp()`), and `process_memory()` records a pts only when it is `> 0`. A cursor-only refresh still carries no pts and is stamped `now()` - never the frame's pts, which would duplicate a timestamp - so the duplicate filter still cannot drop it; a later frame is never stamped earlier than such a refresh (its compositor pts can predate the refresh's `now()`, and the RTP timestamp must not go backwards), and a frame that arrives before a pending refresh is handed out supersedes it and keeps its own pts (`MeowCursorPipewireTest.*Pts*`, `MeowCursorPipewireTimestamp.*`). The viewport idle re-convert in `video.cpp` never inherits a pts: `encode_run()` only takes `frame_timestamp` from an image it popped this iteration, so a re-converted frame goes out with none and `stream.cpp` substitutes the rate-control time, exactly like upstream's own duplicate frames. | 2026-09-24 |
...
| `src/platform/linux/cuda.cpp`, `src/platform/linux/cuda.cu`, `src/platform/linux/cuda.h` | `MEOW-TOUCH(viewport-cuda)` (cropped upload, 2026-09-24) | **Upload only what a crop samples.** `cuda_ram_t::convert()` copied the whole captured frame (30.9 MB on the 5360x1440 desktop) to the GPU on every frame. `cuda_t::meow_viewport_apply()` now records `meow::viewport::cuda_upload_rect()` for the configuration it just set, and the two `sws.load_ram(img, tex.array)` calls in `cuda_ram_t::convert()` become `meow_viewport_load(img, tex.array)` (**the file's 2 deletions**: those two lines, modified in place), which uploads the whole frame exactly as before when nothing is cropped and only the cropped span plus a 2-texel sampling margin otherwise, via a new `sws_t::load_ram_region()` in `cuda.cu`/`cuda.h` (additions only; `cuda.cu`'s deletion count stays 11). The texels land at their own coordinates, so the kernels' source map is untouched and stale texels outside the span are never sampled - `MeowViewportCuda.EveryTexelAKernelCanSampleIsUploaded` checks that over 5000 random crops. Measured with `tools/meow/cuda_upload_probe.cu` on the RTX 5050 (median of 300 interleaved copies): full frame 3.0-4.6 ms, 16:9 HDMI crop 1.2-1.4 ms, 2x phone zoom 0.6 ms. Also on 2026-09-24: `meow_viewport_init()` is no longer gated on `meow_viewport_following` - the geometry is needed to answer requests with the full frame when cropping is off and to map cursor positions (4 of our own lines removed). | 2026-09-24 |
...
| `src/config.h` | `MEOW-TOUCH(cursor)` | **One line**: `bool cursor_reporting` next to `viewport_following`, same reasoning as the row above. The `adaptive-bitrate` row's block also gained `bool adaptive_bitrate` (the explicit off switch) on 2026-09-24. | 2026-09-24 |
...
| `src/config.cpp` | `MEOW-TOUCH(cursor)` | **Two lines**: `true, // cursor_reporting` in the `video` initializer and `bool_f(vars, "meow_cursor_reporting", ...)`. 2026-09-24 also flipped `viewport_following` to `true`, added `true, // adaptive_bitrate` plus `bool_f(vars, "meow_adaptive_bitrate", ...)` to the `adaptive-bitrate` lines, and changed the meaning of `adaptive_bitrate_min/max = 0` to "automatic" (spec D1: every meow feature on by default, each with an off switch). | 2026-09-24 |
| `src/stream.cpp` | `MEOW-TOUCH(meow-control)` | **2026-09-24, replaces the separate `adaptive-bitrate` and `viewport` hooks in this file (70 added, 1 deleted, down from 102/0).** One include (`src/meow/control_stream.h`), a `control_meow_t` framing struct and `send_meow_control()` (frame, encrypt, send - the only thing `src/meow/` cannot do, because `control_header_v2`, the `#pragma pack(1)` region, `encode_control()` and `control_server_t` are private to this file), one `meow::control::session_state_t meow` member on `session_t` (per-session state with exactly the session's lifetime; there is no other owner), one `register_handlers()` call replacing the viewport registration and the inline `0x5502` handler, one `on_session_tick()` call in the existing per-session loop (echo, APPLIED, cursor POSITION, ENet RTT sampling), three one-line length checks at the top of the upstream `IDX_LOSS_STATS`, `IDX_INVALIDATE_REF_FRAMES` (spec F5) and `IDX_INPUT_DATA` handlers (the first two read fixed-size payloads with no length check, the third trusts a client-supplied length prefix and decrypts past the end of the payload - authenticated heap over-reads), and the loop's `server->iterate(150ms)` wrapped in `meow::control::iterate_timeout()` (**the one deleted line**: it returns `150ms` unless a subscribed client is following an active cursor source or a viewport echo is owed, then `8ms`, so POSITION can run at 60 Hz and an echo reaches the client within a frame of the frame it names). Layers 1-3 fail for the reasons above: every one of these is either a member of a private type or a call site inside `controlBroadcastThread()`, a free function. Every decision - packet numbers and collision checks, parsing and length validation, what to send and when, coalescing, the RTT cadence - is in `src/meow/control_stream.h` and unit tested against stand-in server/session types in `tests/unit/meow/test_control_stream.cpp`. | 2026-08-25 |
| `src/stream.cpp` | `MEOW-TOUCH(viewport)` | **One line left**: `meow::viewport::reset()` at the end of `controlBroadcastThread()`, so a crop cannot leak into the next broadcast. The echo, its framing and the registration moved to `MEOW-TOUCH(meow-control)` above on 2026-09-24; the echo is now produced on the encode path (`src/video.cpp`) and only *sent* from here. | 2026-08-25 |
| `src/video.cpp` | `MEOW-TOUCH(adaptive-bitrate)` | An include, two lines constructing a `meow::adaptive_bitrate::governor_t` in `encode_run()` (placed after `fail_guard` so the guard, which can move the session away on teardown, destructs before the governor borrowing its codec context; since 2026-09-24 it takes a `governor_config_t` carrying `meow_adaptive_bitrate`, the automatic-floor/ceiling settings, `max_bitrate` and the FEC percentage), and one `ab_governor.tick()` call in the encode loop. Layers 1-3 fail because `avcodec_encode_session_t` is declared inside `video.cpp`, is not exported by `video.h`, and `encode_run()` is a free function in `namespace video` with no virtual to override - so the live `AVCodecContext` is reachable from nowhere else. **Every decision is layer 1**: the controller in `src/meow/adaptive_bitrate.h` is pure and unit tested without a GPU; the governor in `src/meow/adaptive_bitrate_encoder.h` only copies its answer onto the codec context and is tested against a real (unopened) `AVCodecContext`. | 2026-08-25 |
...
| `src/video.cpp` | `MEOW-TOUCH(viewport)` (encode loop) | **2026-09-24, four lines in `encode_run()`, `0` deletions:** declare a `meow::viewport::encode_hook_t<platf::img_t>`, `remember(img)` after a successful `convert()`, `refresh(*session)` before `encode()` (re-converts the last captured image once per new viewport request, so a pan works on an idle desktop where KWin delivers no frames), and `encoded(frame_nr - 1)` after it (publishes the echo v2 naming that frame). Layers 1-3 fail because the image, the session and the frame number exist only on `encode_run()`'s stack. The steady-state cost is a `shared_ptr` copy and a few relaxed atomic loads; all logic is in `src/meow/viewport_runtime.h` and tested in `MeowViewportSession.*`. | 2026-09-24 |
| `src_assets/common/assets/web/configs/config_tabs.json` | (no marker — JSON) | **`5` added, `0` deleted.** Five keys in the `av` tab's defaults. `viewport`: `meow_viewport_following` after `output_name` (default `enabled` since 2026-09-24). `cursor`: `meow_cursor_reporting` after it (`enabled`). `adaptive-bitrate`: `meow_adaptive_bitrate` (`enabled`), `adaptive_bitrate_min` and `adaptive_bitrate_max` (`0` = automatic) after `max_bitrate`. These are the defaults the Web UI shows, so they match the C++ defaults in `config.cpp` exactly (the smoke run reads them back through `/api/config`). Ordering matches `configuration.md`, as `ConfigConsistencyTest.ConfigOptionsInSameOrderWithinSections` requires; a new option that is absent here fails the gate. **Moved here on 2026-09-24:** these keys used to sit in the JS defaults object inside `config.html`; upstream `d23336dc` replaced that page with the `Config.vue` SPA component and moved all option defaults into this file. `Config.vue` itself is upstream's, unmodified. JSON admits no comment, so this row is the declaration. | 2026-08-25 |
| `src_assets/common/assets/web/configs/tabs/audiovideo/DisplayModesSettings.vue` | (no marker — Vue SFC) | A `Checkbox` import, and after the existing `max_bitrate` input a `meow_adaptive_bitrate` checkbox (default on, 2026-09-24) followed by the two `<input type="number">` blocks for `adaptive_bitrate_min/max`. Without them the settings exist in the config file and the locale file but are unreachable from the web UI. Appended after the last existing field so the merge seam is small. | 2026-08-25 |
| `src_assets/common/assets/web/configs/tabs/AudioVideo.vue` | `MEOW-TOUCH(viewport)`, `MEOW-TOUCH(cursor)` | Two `<Checkbox>`es between `DisplayOutputSelector` and `DisplayDeviceOptions`, matching the documented option order: `meow_viewport_following` (its `default` flipped to `true` on 2026-09-24) and `meow_cursor_reporting` (default `true`). Without them the settings exist but have no control. | 2026-08-25 |
| `src_assets/common/assets/web/public/assets/locale/en.json` | (no marker — JSON) | Ten strings total, label + description for each meow setting: `meow_adaptive_bitrate`, `adaptive_bitrate_min`, `adaptive_bitrate_max`, `meow_cursor_reporting`, `meow_viewport_following` (the descriptions were rewritten on 2026-09-24 for the new defaults and the automatic floor/ceiling). Required by `ConfigConsistencyTest.AllConfigOptionsExistInAllFiles`. English only, per the upstream rule in `AGENTS.md`. | 2026-08-25 |
| `docs/configuration.md` | (no marker — prose only) | Documents the reserved `output_name = all` value next to the existing `output_name` description, and (2026-08-25) adds the `meow_viewport_following` section after it. A separate page would leave the settings undocumented where users actually look, and the consistency test requires this one to be here. `adaptive-bitrate` additionally documents `adaptive_bitrate_min` and `adaptive_bitrate_max` next to the existing `max_bitrate`, which is where a user comparing the two will look. `tests/integration/test_config_consistency.cpp` requires every option in `config.cpp` to appear here, in `configs/config_tabs.json` (formerly `config.html`) and in `en.json`, **in the same order within a section** — so these are a hard requirement of the gate, not a choice.  2026-09-24: `meow_cursor_reporting` after `meow_viewport_following`, `meow_adaptive_bitrate` after `max_bitrate`, and the viewport/adaptive sections rewritten for their new on-by-default behaviour. | 2026-08-25 |
...
```bash
mb=$(git merge-base origin-upstream/master HEAD)
git diff --numstat $mb HEAD -- src/ | awk '$2>0'      # every file listed must have a row above that explains its deletions
...
file, `docs/meow/viewport-following.md`, `src/meow/*.h`, `tests/unit/meow/*.cpp` and
`tools/meow/*` (hardware probes, not part of the build).
...
1	0	.gitignore
17	0	AGENTS.md
287	8	src/platform/linux/kwingrab.cpp    # 8 = in-place signature/comment edits
41	1	src/platform/linux/pipewire.cpp    # cursor: the params array grew from 3 to 4
<n>	4	src/platform/linux/misc.cpp        # rebrand: the four config-dir literals, replaced in place
<n>	0	src/platform/linux/cuda.h
36	11	src/platform/linux/cuda.cu         # 11 = deliberate; MUST NOT GROW
<n>	2	src/platform/linux/cuda.cpp        # viewport-cuda: the two load_ram() calls -> meow_viewport_load()
<n>	0	src/config.h
<n>	3	src/config.cpp                     # rebrand: state/config/log file names replaced in place
<n>	1	src/stream.cpp                     # meow-control: iterate(150ms) -> iterate_timeout(150ms)
<n>	0	src/video.cpp
