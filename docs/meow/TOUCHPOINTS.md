...
| `src_assets/common/assets/web/index.html`, `src_assets/common/assets/web/apps.html`, `src_assets/common/assets/web/template_header.html`, `src_assets/common/assets/web/Navbar.vue`, `src_assets/common/assets/web/NavbarSimple.vue`, `src_assets/common/assets/web/ResourceCard.vue`, `src_assets/common/assets/web/configs/tabs/General.vue`, `src/platform/macos/misc.mm` | `MEOW-TOUCH(web-identity)` | **The web UI told the user it was Sunshine.** Reported symptom: the update banner announced *"A new Stable Version is Available!"* comparing our build against **LizardByte/Sunshine's** releases — a different product's version numbers. All logic lives in a new layer-1 file, `src_assets/common/assets/web/meow_release_check.js`; the upstream pages keep only an `import` and a call, which is layer 3. **The endpoint swap alone was not enough:** `meowerse/sunmeow` publishes no releases, so both endpoints 404. A 404 body is a truthy object, so `new SunshineVersion(body)` does not throw — it yields `version: undefined` and the existing computed guards happen to suppress the banner. But the pre-release path called `.find()` on that object, and a 404 body has no `.find`, so it threw a `TypeError` mid-`created()` and skipped everything after it in the same `try` (on Windows, the virtual-input status and licence fetches). The new module treats "no release feed" as a **normal state**, returning `null` so the null guards suppress the banner, and never announces an update it cannot substantiate. Also fixed: the browser tab `<title>` (the most-seen label in the UI), the navbar tooltip and image `alt` (what a screen reader announces), and the LICENSE/NOTICE buttons, which linked to *upstream's* copies rather than the terms shipped with this binary — both GPL-3.0 and inherited verbatim, but they must describe this build. `src/platform/macos/misc.mm` returned `~/.config/sunshine`: known pre-existing rebrand debt, deliberately held out of the security sync to keep that diff reviewable, fixed here. **Deliberately unchanged:** the `sunshine_name` config KEY (renaming it breaks every existing config and `ConfigConsistencyTest`), the `sunshine_version.js` class name and `sunshine.css` (internal identifiers, invisible to users, and renaming them would conflict on every future sync), and the logo artwork — the `.png`/`.ico`/`.svg` assets are still Sunshine's drawings, which is a design task, not a code one. NB `General.vue`'s placeholder was already inaccurate upstream: the true default is `platf::get_host_name()`, not the product name; only the shown name changed. | 2026-09-03 |
