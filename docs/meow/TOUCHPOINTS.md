...
Nine upstream **C++ sources** are now modified: two by `unified-desktop-capture`, four by
`viewport` (two of them one and two lines respectively, to register a setting), and three by
`viewport-cuda`. An earlier revision of this file said *"Zero `MEOW-TOUCH` markers exist in
`src/`"* and described that as the state to keep. The first half stopped being true the moment
whole-desktop capture landed; the second half is still the goal.
`git grep -n 'MEOW-TOUCH' -- src/ src_assets/` is the authority — if this table disagrees with
it, this table is wrong.
...
policy they hook into live in `src/meow/display_union.h`, `src/meow/viewport.h` and
