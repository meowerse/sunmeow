...
`git grep -n 'MEOW-TOUCH' -- . ':!*.md'` is the authority — if this table disagrees
...
| `tests/tests_main.cpp` | `MEOW-TOUCH(test-sandbox)` | **`5` added, `1` deleted** — one include, whose `constructor(101)` arms the redirect. All logic is layer 1, in the new `tests/meow/test_sandbox.h`. **Why a constructor and not `main()`:** the first version of this put the call as `main()`'s first statement and **did not work** — the banner printed, the temp dir appeared, and a full run still rewrote the real state file. `platf::appdata()` memoises behind `std::call_once`, and `src/config.cpp:62` resolves it at *namespace scope* (`const std::string APPS_JSON_PATH = platf::appdata().string() + "/apps.json";`), so the first call happens during static initialisation, before `main()` is entered. `constructor(101)` runs ahead of the unprioritised constructors that carry a TU's dynamic init. **What it fixes:** the suite wrote into the developer's real config directory — `test_http_pairing.cpp` drives the genuine pairing code and overwrote `sunshine_state.json` with a fixture device named `"test"`. Inherited, not ours: reproduced on an unmodified baseline. **Proven:** a full 570-test run with *no* env overrides leaves the real state file byte-identical while `PairingTest` runs 24 times and its writes land in `/tmp/sunshine-tests-<pid>/sunshine/` — the sandbox receiving a `sunshine/` subdirectory is the exact evidence the failed first attempt could not produce. | 2026-08-26 |
| `cmake/targets/common.cmake` | `MEOW-TOUCH(ccache-scope)` | **`10` added, `0` deleted** — two one-line calls plus their comments, around upstream's existing `target_compile_definitions(sunshine PUBLIC ${SUNSHINE_DEFINITIONS})`. All logic is layer 1, in the new `cmake/meow/version_definitions.cmake`. Layers 1–3 fail because the list is a plain CMake variable that upstream builds in `cmake/prep/build_version.cmake` and consumes here: nothing can intercept a variable between two `include()`s from outside the file that reads it, and the re-application must land *after* `add_subdirectory(tests)`, which only this file can sequence. **Why it exists:** `PROJECT_VERSION` carries a `-<short-sha>` suffix on every branch not named `master`, and CI checkouts are detached — so upstream's global `target_compile_definitions` put the commit identity on all 349 objects' command lines, and ccache hashes the command line. Measured 0/349 hits on a commit that changed only `ci.yml`. After this, 3 of 352 TUs carry it. | 2026-08-26 |
...
`web-deps` markers — **16** distinct (file, marker) pairs. Reconcile them against the table
...
lands in a new part of the tree.

**The enumerated pathspec has been replaced, because enumerating was the bug.** It began as
`src/ src_assets/`, then had to grow `vite.config.js` when a marker landed at the repo root,
then `cmake/` for `ccache-scope`, and would have needed `tests/` for `test-sandbox`. Every one
of those gaps was silent: the marker existed and the pre-sync check reported nothing, which
reads as a clean bill of health rather than as "I did not look there". The grep is now
repo-wide with documentation excluded:

```bash
git grep -n 'MEOW-TOUCH' -- . ':!*.md'
```

Markers live in source and config files, never in Markdown — and the `':!*.md'` exclusion is
load-bearing rather than cosmetic: both this file and `CLAUDE.md` quote the marker syntax in
illustrative code blocks (`MEOW-TOUCH(clipboard)` in CLAUDE.md §3 is an example, not a
touch-point), so a naive repo-wide grep reports false positives and trains you to ignore it.
