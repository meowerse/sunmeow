...
| `tests/tests_main.cpp` | `MEOW-TOUCH(test-sandbox)` | **`6` added, `0` deleted** — one include and one call as the first statement of `main()`. All logic is layer 1, in the new `tests/meow/test_sandbox.h`. **Why it must be here and nowhere else:** `platf::appdata()` memoises into a function-local `static` behind `std::call_once`, so the first call in the process wins for its lifetime — and `logging::init()`, reached from `SunshineEnvironment::SetUp()`, already touches it. A gtest fixture, a global environment, or a static initialiser in another TU are all either too late or unpredictably ordered. **What it fixes:** the suite wrote into the developer's real config directory — `test_http_pairing.cpp` drives the genuine pairing code and overwrote `sunshine_state.json` with a fixture device named `"test"`, while `test_httpcommon.cpp` and `test_file_handler.cpp` built scratch dirs as `platf::appdata() + "/tests/"`. That costs the user every paired client. Reproduced on an unmodified baseline, so it is inherited, not ours. **Proven:** with the redirect, a full 570-test run leaves `~/.config/sunshine/sunshine_state.json` byte-identical (md5 unchanged) while `PairingTest` runs 24 times and its writes land in the sandbox instead. | 2026-08-26 |
...
and would have had to grow again for `tests/` and `cmake/`. Every one of those gaps was
silent: the marker existed and the pre-sync check reported nothing, which reads as a clean
bill of health rather than as "I did not look there". The grep is now repo-wide with
