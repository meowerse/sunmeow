...
now returns **11** marker families (`adaptive-bitrate`, `ccache-scope`, `cmake-deps`,
`green-brand`, `rebrand`, `test-sandbox`, `unified-desktop-capture`, `viewport`,
`viewport-cuda`, `web-deps`, `web-identity`) across **65** distinct (file, marker) pairs in
**59** files, as of 2026-09-24 on the `web-identity` branch. (An earlier revision listed a
`clipboard` family; no such marker exists — it is only the example in `CLAUDE.md` §3.)
...
<n>	<m>	src_assets/common/assets/web/{index,apps,template_header}.html  # web-identity: in-place replacements
